...
class AuthViewModel @Inject constructor(
    ...
    private val _authResult = MutableLiveData<Resources<AuthToken>>()
    val authResult: LiveData<Resources<AuthToken>> get() = _authResult

    fun login(email: String, password: String) {
        viewModelScope.launch {
            _authResult.postValue(Resources.loading(null))
            authRepository.login(
                AuthLoginRequest(email = email, password = password)
            ).collect { _authResult.postValue(it) }
        }
    }
    ...
    fun checkPreviousAuthUser() {
        viewModelScope.launch {
            _authResult.postValue(Resources.loading(null))
            authRepository.checkPrevAuthUser().collect { _authResult.postValue(it) }
        }
    }
